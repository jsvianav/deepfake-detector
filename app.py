"""Deepfake Detector — Gradio interface with Veritas premium design.

Loads both models once at startup and serves inference through a Blocks UI
that accepts drag-and-drop media uploads and returns animated HTML result cards.
"""
...
# ---------------------------------------------------------------------------
# SVG icons (HTML attribute names, not React camelCase)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CSS — Veritas dark theme (from Chat.html design bundle)
# ---------------------------------------------------------------------------
CUSTOM_CSS = """
...
/* ── Container with atmospheric gradient ─────────────────── */
.gradio-container {
...
/* Bot bubble — transparent so HTML cards show through */
#chatbot .message.bot,
...
def _mini_bar(score_val: Optional[float], label: str, icon: str) -> str:
    ...
    if score_val is None:
        return (
            ...
            + "<div style=\"font-size:10px;color:#6B6883;font-family:'JetBrains Mono',monospace\">no data</div>"
        )
    ...
    return (
        ...
        + f'<span style="font-size:13px;color:#6B6883;margin-left:1px">'
        + f'{p:.1f}</span><span style="font-size:12px;color:#6B6883">%</span></div>'
...
def _format_result(result: DetectionResult, file_name: str) -> str:
    """Render a DetectionResult as an animated HTML result card."""
    ...
    if score >= 0.6:
        kind, verdict_label     = "fake", "Synthetic content"
        ...
        kind, verdict_label     = "real", "Authentic content"
        ...
        kind, verdict_label     = "amb", "Inconclusive"
    ...
    # ── Breakdown section (video only) ──────────────────────────────────
    breakdown = ""
    if result.file_type == "video":
        audio_score = result.audio_score if result.had_audio else None
        audio_meta  = "wav2vec2 · 16 kHz" if result.had_audio else "no audio track"
        breakdown = (
            ...
            + f"<div style=\"font-size:10px;color:#6B6883;font-family:'JetBrains Mono',monospace\">{result.frame_count} frames · 1 fps</div>"
            + '</div>'
    ...
    tech = _tr("fused_score", f"{result.fused_score:.4f}")
    if result.video_score is not None:
        tech += _tr("video_score", f"{result.video_score:.4f}")
    if result.audio_score is not None:
        tech += _tr("audio_score", f"{result.audio_score:.4f}")
    tech += _tr("frame_count", str(result.frame_count))
    tech += _tr("had_audio",   str(result.had_audio).lower())
    tech += _tr("file_type",   result.file_type)
    tech += _tr("device",      device)
    ...
    return (
        ...
        # Top bar: icon + filename + type chip
        ...
        f'<div style="font-size:12px;color:#6B6883;margin-top:8px">probability of synthetic content</div>'
        f'</div>'

        # Animated scale bar with needle
        ...
        f'<span style="color:#6EE7B7">0% — Authentic</span>'
        f'<span style="color:#6B6883">50</span>'
        f'<span style="color:#FCA5A5">100% — Synthetic</span>'
        f'</div></div>'
        ...
        + _ICON_ALERT +
        '<span>Detection is probabilistic, not definitive. Treat the verdict as one signal among many — '
        'review forensic details and your own context before drawing conclusions.</span>'
        '</div>'
...
def analyse_file(file_obj, history: ChatHistory) -> Tuple[ChatHistory, ChatHistory, None]:
    if file_obj is None:
        history = history + [("", "⚠️ Please upload a file before analyzing.")]
        return history, history, None

    file_path = file_obj if isinstance(file_obj, str) else getattr(file_obj, "name", str(file_obj))

    if not file_path or not Path(file_path).is_file():
        history = history + [("", "⚠️ Invalid file. Please upload a video, audio, or image file.")]
        return history, history, None
    ...
    try:
        ...
        bot_msg = f"❌ Unexpected error: {exc}"
...
HEADER_HTML = (
    ...
    'Deepfake Detector</span></div>'
    '<div style="font-size:12px;color:#6B6883;margin-top:3px">ViT image · Wav2Vec2 audio · 100% local inference</div>'
    '</div>'
...
WELCOME_HTML = (
    ...
    + _ICON_SEARCH +
    '</div>'
    '<div style="font-size:14px;font-weight:600;color:#F8F7FF;letter-spacing:-.01em">Ready when you are</div>'
    '</div>'
    '<p style="font-size:13px;color:#9CA3AF;line-height:1.65;margin:0 0 16px 0">'
    'Drop a <strong style="color:#C4B5FD">video</strong>, <strong style="color:#C4B5FD">audio</strong>, or '
    '<strong style="color:#C4B5FD">image</strong> file into the panel on the right, then click '
    '<strong style="color:#C4B5FD">Analyze</strong>. Everything runs locally — your media never leaves this machine.'
    '</p>'
    '<div style="display:flex;flex-wrap:wrap;gap:7px">'
    + _chip("mp4 · mov · avi · webm") + " "
    + _chip("mp3 · wav · m4a · ogg")  + " "
    + _chip("jpg · jpeg · png")        + " "
    + _chip("max 100 MB",  "#6EE7B7", "rgba(16,185,129,.08)", "rgba(16,185,129,.2)") +
    '</div>'
    '</div>'
)
...
with gr.Blocks(
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=1, elem_classes=["right-panel"]):
            ...
            analyse_btn = gr.Button("Analyze", variant="primary", size="lg")
            ...
            gr.Markdown(
                ...
                "100% local · no external APIs · open source"
            )
